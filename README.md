...
### Connection Pool 개수에 따른 비교
|테스트 번호|시도 횟수|캐시 개수|데이터 개수|테스트케이스|동시접속수|Connection Pool 개수|예상 실패 건수|실제 실패 건수|오차율|(실패)데이터베이스 시간 초과|(실패) CB 에러 수|비고|
|---|---|---|---|---|---|---|---|---|---|---|---|---|
|4|1번|0개|1개|2000개|100개|200개|약 1416개|1833개|약 -29%|1666개|167개|커넥션 Pool 200개 경우|
|5-1|1번|0개|1개|2000개|100개|Default(10개)|약 1416개|1829개|약 -29%|1667개|162개|커넥션 Pool 기본값(10개)인 경우|
|5-2|1번|0개|1개|2000개|100개|Default(10개)|약 1416개|1845개|약 -29%|1690개|155개|커넥션 Pool 기본값(10개)인 경우|

- Connection Pool의 개수를 늘려도 크게 차이가 보이지 안음

### 동시 접속 수에 따른 비교
|테스트 번호|시도 횟수|캐시 개수|데이터 개수|테스트케이스|동시접속수|Connection Pool 개수|예상 실패 건수|실제 실패 건수|오차율|(실패)데이터베이스 시간 초과|(실패) CB 에러 수|비고|
|---|---|---|---|---|---|---|---|---|---|---|---|---|
|4|1번|0개|1개|2000개|100개|200개|약 1416개|1833개|약 -29%|1666개|167개|동시접속 100개|
|6|1번|0개|1개|2000개|200개|200개|약 1416개|1905개|약 -35%|1821개|84개|동시접속 200개|

- 동시 접속이 더 많이 생길 수록 오류의 개수는 증가,데이터베이스 접속이 불가능해서 시간 초과가 더 많이 발생

### Retry 수에 따른 비교
|테스트 번호|시도 횟수|캐시 개수|데이터 개수|테스트케이스|동시접속수|Connection Pool 개수|예상 실패 건수|실제 실패 건수|오차율|(실패)데이터베이스 시간 초과|(실패) CB 에러 수|비고|
|---|---|---|---|---|---|---|---|---|---|---|---|---|
|4|1번|0개|1개|2000개|100개|200개|약 1416개|1833개|약 -29%|1666개|167개|Retry 1번|
|7-1|2번|0개|1개|2000개|200개|200개|약 1246개|1830개|약 -47%|1752개|78개|Retry 2번|
|7-2|2번|0개|1개|2000개|200개|200개|약 1246개|1826개|약 -47%|1756개|70개|Retry 2번|

- Retry을 증가하면서 CB에러 확률이 절반으로 감소, 데이터베이스 시간 초과 수는 대동소이

### Cache 따른 비교
|테스트 번호|시도 횟수|캐시 개수|데이터 개수|테스트케이스|동시접속수|Connection Pool 개수|예상 실패 건수|실제 실패 건수|오차율|(실패)데이터베이스 시간 초과|(실패) CB 에러 수|비고|
|---|---|---|---|---|---|---|---|---|---|---|---|---|
|7-1|2번|0개|1개|2000개|200개|200개|약 1246개|1830개|약 -47%|1752개|78개|Cache 없는 경우|
|7-2|2번|0개|1개|2000개|200개|200개|약 1246개|1826개|약 -47%|1756개|70개|Cache 없는 경우|
|8-1|2번|3개|5개|2000개 * 5|200개 * 5|200개|???|7개||0개|7개|캐시 3개 및 데이터 개수 5개 동시 실행|
|8-2|2번|3개|5개|2000개 * 5|200개 * 5|200개|???|102개||75개|27개|캐시 3개 및 데이터 개수 5개 동시 실행|
|9|2번|3개|5개|2000개 * 5|200개 * 5|Default(10개)|???|2개||0개|2개|캐시 동기화 기능 추가(LinkedList)|

- 캐시를 추가하면서 에러 확률이 급격히 감소
- CB 모듈에서 데이터를 요청하는 동안 다른 이미 가지고 있는 데이터들을 캐시를 통해서 보내지므로 빠르게 처리 완료
...
- 기존: CB 모듈에 들어가기 전에 데이터베이스 트랜잭션을 들어간 상태에 CB 모듈 실행
- 변경: CB 모듈이 완료가 된 후에 트랜잭션에 접속
- 트랜잭션으로 데이터베이스 접속오류가 매우 많았는데 데이터베이스 시간초과 감소를 매우 낮춤
- 에러 오차율을 +로 만들면서 기능 향상

### Cache 타임아웃 기능 추가 후 결과 비교
|테스트 번호|시도 횟수|캐시 개수|데이터 개수|테스트케이스|동시접속수|Connection Pool 개수|예상 실패 건수|실제 실패 건수|오차율|(실패)데이터베이스 시간 초과|(실패) CB 에러 수|비고|
|---|---|---|---|---|---|---|---|---|---|---|---|---|
|15-1|1번|3개|5개|2000개|100개|Default(10개)|약 1416개|293개|약 79%|87개|204개|Retry 1번 제한, 타임아웃 10초|
|15-2|1번|3개|5개|2000개|100개|Default(10개)|약 1416개|483개|약 66%|128개|355개|Retry 1번 제한, 타임아웃 10초|
|16-1|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|491개|약 61%|140개|349개|Retry 2번 제한, 타임아웃 10초|
|16-2|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|479개|약 62%|116개|363개|Retry 2번 제한, 타임아웃 10초|
|17-1|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|373개|약 70%|110개|256개|캐시 타임아웃 1초, 9개 IOException 에러|
|17-2|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|402개|약 68%|90개|360개|캐시 타임아웃 1초, 42개 IOException 에러|
|18-1|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|293개|약 76%|82개|208개|캐시 스레드 제거, TimeStamp 사용, 2개 IOException 에러|
|18-2|2번|3개|5개|2000개|100개|Default(10개)|약 1246개|291개|약 76%|65개|219개|캐시 스레드 제거, TimeStamp 사용, 5개 IOException 에러|

- 타임아웃 기능이 추가되면서 큰 차이가 발생하지 않음
- 타임아웃이 되면서 새로운 IOExcepion이 발생
- Http 요청을 하게 되면 Response에서 Header가 Transfer-encoding이 Chunk으로 오는 오류가 발생
