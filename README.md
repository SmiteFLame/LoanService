...
### 서브 시나리오
```
1a. limit 혹은 offset이 입력되지 않은 경우
    1. "필요한 파라미터 조건이 없습니다." 메시지를 저장한다.
    2. 400(BAD_REQUEST) 상태를 전송한다.
1b. limit 혹은 offset이 정수값으로 입력되지 않은 경우
    1. "필요한 파라미터 조건이 없습니다." 메시지를 저장한다.
    2. 400(BAD_REQUEST) 상태를 전송한다.
1c. limit가 양수가 아니거나 offset이 음수값이 들어온 경우
...
2. 이미 검색된 신용 정보가 존재한지 확인한다.
3. 신용평가 모듈에 신용등급 조회를 요청한다.
4. 유저의 정보가 존재한지 확인한다.
5. CB 모듈에게 신용등급 조회를 요청한다.
6. 신용동급을 바탕으로 대출 가능 여부를 판단한다.
7. 신용평가 조회 기록과 결과를 신용평가 데이터베이스에 저장한다.
8. 신용평가 모듈에서 대출 서비스 모듈로 결과값을 전달한다.
9. 신용평가 결과 값을 저장한다.
10. 사용자에게 신용평가 결과 값과 201(CREATED) 상태를 전송한다.
```
...
1c. limit가 양수가 아니거나 offset이 음수값이 들어온 경우
    1. "잘못된 페이지 조건이 입력되었습니다" 메시지를 저장한다.
    2. 400(BAD_REQUEST) 상태를 전송한다.
1d. id-type입력값이 "account-id"인 경우
